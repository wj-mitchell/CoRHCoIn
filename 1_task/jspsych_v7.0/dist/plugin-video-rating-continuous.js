var jsPsychVideoRatingContinuous = (function (jsPsych) {
  ...
  class VideoRatingContinuousPlugin {
    ...
    trial(display_element, trial) {
      ...
      if (trial.reference_img !== null) {
        html += '<div id="jspsych-video-rating-continuous-reference-img" style="position: absolute; top: 10px; left: 50%; transform: translateX(-50%); z-index: 1000;">';
        html += '<img src="' + trial.reference_img + '" style="width: auto; height: 125px; object-fit: cover; border: 2px solid yellow;">';
        html += '</div>';
      }
    
      var video_html = '<video id="jspsych-video-rating-continuous-stimulus-video" style="';
      if (trial.width) {
        video_html += 'width:' + trial.width + 'px; ';
      } else {
        video_html += 'width: 100%; ';  // Default behavior
      }
      if (trial.height) {
        video_html += 'height:' + trial.height + 'px; ';
      } else {
        video_html += 'height: auto; ';  // Default behavior
      }
      video_html += '"';
